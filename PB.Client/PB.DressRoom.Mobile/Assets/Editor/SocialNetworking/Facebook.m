...
static NSString *kFacebookToken = @"kFacebookToken";
static NSString *kFacebookExpiration = @"kFacebookExpiration";
...
- (id)init
{
	if( ( self = [super init] ) )
	{
		...
		NSString *token = [defaults objectForKey:kFacebookToken];
		NSDate *expirationDate = [defaults objectForKey:kFacebookExpiration];

		NSHTTPCookieStorage *cookies = [NSHTTPCookieStorage sharedHTTPCookieStorage];
		NSArray *facebookCookies = [cookies cookiesForURL:[NSURL URLWithString:@"http://login.facebook.com"]];
		
		if( facebookCookies.count && token && expirationDate && [expirationDate compare:[NSDate date]] == NSOrderedDescending )
		{
...
- (void)fbDialogLogin:(NSString *)token expirationDate:(NSDate *)expirationDate {
  ...
  self.expirationDate = expirationDate;
	...
	// store token info
	NSUserDefaults *defaults = [NSUserDefaults standardUserDefaults];
	[defaults setObject:token forKey:kFacebookToken];
	[defaults setObject:expirationDate forKey:kFacebookExpiration];
