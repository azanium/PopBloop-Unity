...
public class PBThirdPersonController : MonoBehaviour 
{
	#region MemVars & Props

    private static PBThirdPersonController pbThirdPersonController;
    public static event Action OnPlayerMovementStopped = null;
	...
	void Update() 
    {
        ...
        if (Input.GetKey(KeyCode.Space) && _motionType == MotionType.MoveToInteract)
        {
        ...
        {
            if (_motionType == MotionType.Drag || _motionType == MotionType.MoveToInteract || _motionType == MotionType.Interacting)
            {
                StopDragMove();
            }
		...
		{
            if (_motionType != MotionType.Interacting)
            {
                collisionFlags = controller.Move(movement);
            }
            else
            {
                string animation = PBConstants.ANIM_SIT;
                if (_activeInteractionType == ItemInteraction.ItemInteractionType.Sleep)
                {
                    animation = PBConstants.ANIM_LIE;
                }
                PlayerAnimator.playerAnimation.Animate(animation, AnimationAction.Play, WrapMode.Loop, 1f, 0);
                transform.position = Vector3.Lerp(transform.position, _activeInteractionTransform.position, Time.deltaTime * runSpeed);
            }
		...
		if (IsGrounded())
		{
		    ...
		    if (InputManager.IsKeyClearForGameControl)
			{
                if (_motionType == MotionType.Interacting)
                {
                    transform.rotation = Quaternion.Slerp(transform.rotation, _activeInteractionTransform.rotation, Time.deltaTime * rotateSpeed);
                }
                else
                {
    ...
    {
        if (Mathf.Abs(Vector3.Distance(transform.position, targetDragPosition)) > 0.1f)
        {
    ...
    public void StopDragMove()
    {
        if (_motionType == MotionType.Drag || _motionType == MotionType.MoveToInteract || _motionType == MotionType.Interacting)
        {
            ...
            if (OnPlayerMovementStopped != null)
            {
                OnPlayerMovementStopped();
            }
        }

        _characterState = CharacterState.Idle;
        if (_motionType != MotionType.MoveToInteract)
        {
            _motionType = MotionType.WASD;
        }
        moveSpeed = 0;
    ...
    private void UpdateDragMovement()
    {
        ...
        if (_motionType == MotionType.Drag || _motionType == MotionType.MoveToInteract)
        {
	...
	public bool IsMoving()
	{
        ...
        bool dragMove = ((_motionType == MotionType.Drag || _motionType == MotionType.MoveToInteract) && (_characterState == CharacterState.Walking || _characterState == CharacterState.Running || _characterState == CharacterState.Trotting));
    ...
    public void OnTriggerEnter(Collider collider)
    {
        ...
        if (_motionType == MotionType.MoveToInteract && _activeInteractionObject.collider == collider)
        {
            _motionType = MotionType.Interacting;
            return;
        }
    }
    ...
    private void OnEnable()
    {
        ...
        Messenger<ItemInteraction, Transform, ItemInteraction.ItemInteractionType>.AddListener(Messages.PLAYER_INTERACTO, OnPlayerInteractTo);
    }
    ...
    private void OnDisable()
    {
        ...
        Messenger<ItemInteraction, Transform, ItemInteraction.ItemInteractionType>.RemoveListener(Messages.PLAYER_INTERACTO, OnPlayerInteractTo);
    }
    ...
    private ItemInteraction.ItemInteractionType _activeInteractionType = ItemInteraction.ItemInteractionType.Sit;

    private void StartInteraction(GameObject obj, Transform trans, ItemInteraction.ItemInteractionType interactionType)
    {
        ...
        if (collider != null)
        {
            ...
            _motionType = MotionType.MoveToInteract;
        }
    }
    ...
    private void EndInteraction()
    {
        if (_activeInteractionObject != null && _activeInteractionTransform != null)
        {
            ...
            if (collider != null)
            {
                ...
                _activeInteractionObject = null;
            }
        }
    }

    private void OnPlayerInteractTo(ItemInteraction obj, Transform trans, ItemInteraction.ItemInteractionType interactionType)
    {
        StartInteraction(obj.gameObject, trans, interactionType);
    }
    ...
    static public MotionType GetMotionType()
    {
        return pbThirdPersonController._motionType;
    }
