...
public class winPhotoGallery : WindowBase
{
    ...
    public override void DrawUI()
    {
        if (string.IsNullOrEmpty(_tooltip) == false)
        {
            ...
            GUI.Box(new Rect(posX, mousePosition.y + 40, size.x + 3, size.y), _tooltip, _tooltipStyle);
        }
        _tooltip = "";
    }
    ...
    public override void Draw(int id)
    {
        ...
        if (_texture != null)
        {
            ...
            float top = _margin.y + WindowRect.height - _margin.height - 45;

            if (!_isUploading)
            {
                if (GUI.Button(new Rect(WindowRect.width - _margin.width - _margin.x + 10, top, _closeStyle.normal.background.width, _closeStyle.normal.background.height), 
                ...
                if (_uploaded == false)
                {
                    if (GUI.Button(new Rect(WindowRect.width - _margin.width - _margin.x - _closeStyle.normal.background.width, top, _uploadStyle.normal.background.width, _uploadStyle.normal.background.height), 
                        new GUIContent("", " Upload and share to facebook "), _uploadStyle))
                    {
                        _isUploading = true;
                ...
                {
                    GUIUtility.RotateAroundPivot(-4f, new Vector2(WindowRect.width / 2, WindowRect.height / 2));
                    GUI.Label(new Rect(_margin.x, top + 25, 300, 25), "Photo uploaded, please close..", _textStyle);
                }
            }
            else
            {
                GUIUtility.RotateAroundPivot(-4f, new Vector2(WindowRect.width / 2, WindowRect.height / 2));
                GUI.Label(new Rect(_margin.x, top + 25, 300, 25), "Uploading the photo...", _textStyle);
            }            
        }
