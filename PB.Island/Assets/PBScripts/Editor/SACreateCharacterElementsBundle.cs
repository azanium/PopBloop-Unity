    ...
    private static void CreateElement(BuildTarget target)
    {
        ...
        foreach (Object obj in Selection.GetFiltered(typeof(Object), SelectionMode.DeepAssets))
        {
            ...
            foreach (SkinnedMeshRenderer smr in go.GetComponentsInChildren<SkinnedMeshRenderer>(true))
            {
                ...
                string filepath = path + name + "_" + smr.name.ToLower();
        ...
        EditorUtility.DisplayDialog(PBConstants.APP_TITLE, "Character's element(s) creation completed\n" + path, "OK");
    }
